...
async def create_work_item(
    ...
    *,
    ...
    work_item = WorkItem(
        ...
        planner_status=initial_planner_status().value,
        topic_id=topic_id,
        due_at=due_at,
        due_date_explicit=due_date_explicit,
        due_time_explicit=due_time_explicit,
        next_follow_up_at=next_follow_up_at,
        waiting_since=waiting_since,
        completed_at=completed_at,
...
async def append_work_item_event(
    ...
    *,
    ...
    await record_audit_event(
        session,
        actor_kind="telegram" if telegram_update_id is not None else "system",
        action=f"work_item.{parsed_type.value}",
        outcome="success",
        user_id=user_id,
        entity_kind="work_item",
        entity_id=work_item_id,
        correlation_id=str(telegram_update_id)
        if telegram_update_id is not None
        else None,
        safe_metadata={"status": parsed_type.value},
    )
    return event
